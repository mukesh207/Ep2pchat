...
pub async fn post_story(
    ...
    let ciphertext = base64::engine::general_purpose::STANDARD
        ...
        .map_err(|_| (StatusCode::BAD_REQUEST, Json(json!({"error": "Invalid ciphertext base64"}))))?;
    
    let nonce = base64::engine::general_purpose::STANDARD
        .decode(&payload.nonce_b64)
        .map_err(|_| (StatusCode::BAD_REQUEST, Json(json!({"error": "Invalid nonce base64"}))))?;
    ...
    match res {
        ...
        Err(e) => {
            tracing::error!("Failed to post story: {}", e);
            Err((
...
pub async fn get_stories(
    ...
    match res {
        ...
        Err(e) => {
            tracing::error!("Failed to fetch stories: {}", e);
            Err((
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({"error": "An internal error occurred"})),
            ))
